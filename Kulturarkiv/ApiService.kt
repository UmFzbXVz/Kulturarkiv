...
object ApiService {
    ...
    fun init(context: Context) {
        prefs = context.getSharedPreferences("api_service_prefs", Context.MODE_PRIVATE)
        ...
        cacheJson?.let {
            try {
                ...
                Log.e("ApiService", "Failed to load mediaCache: ${e.message}")
            }
        }
    }
    ...
    fun parseUrlComponents(responseData: String?): Triple<String?, String?, String?>? {
        return try {
            ...
            Log.e("ApiService", "Parsing error: ${e.message}")
            null
        }
    }

    fun generateLocalStreamUrl(
        entryId: String,
        flavorId: String,
        fileExt: String,
        ...
        Log.d(
            "ApiService",
            "Generating local stream URL for $entryId and $flavorId with playSessionId: $playSessionId"
        )

        val baseUrl =
            "https://api.kltr.nordu.net/p/397/sp/39700/playManifest/entryId/$entryId/protocol/https"
        val queryParams =
            "?uiConfId=23454143&playSessionId=$playSessionId&referrer=aHR0cHM6Ly93d3cua2IuZGsvZmluZC1tYXRlcmlhbGUvZHItYXJraXZldC9wb3N0L2RzLnR2Om9haTppbzpiNTIxNmJhYi02OTdkLTRlMDEtYWM4Yy00NjM4YmVjMWY4ZmY=&clientTag=html5:v3.17.46"
    ...
    fun getMediaUrlviaAPI(
        ...
        mediaCache[cacheKey]?.let { cachedUrl ->
            Log.d("KalturaData", "Fetching from cache for $cacheKey: $cachedUrl")
            runOnMainThread { onResult(cachedUrl) }
            return
        }

        Log.d("KalturaData", "Fetching from API for $cacheKey")
                    ...
                    if (flavorId != null && fileExt != null) {
                        val mediaUrl = if (forCast) {
                            ...
                            if (playSessionId == null) {
                                Log.e("ApiService", "playSessionId required for local URL")
                                runOnMainThread { onResult(null) }
                                return@let
                            }
                            generateLocalStreamUrl(entryId, flavorId, fileExt, playSessionId)
                        }
                        Log.d("KalturaData", "Generated URL: $mediaUrl")
        ...
        client.newCall(request).enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                if (response.isSuccessful) {
                    ...
                    try {
                        JSONObject(responseData) // Validate JSON
                        callback(responseData)
                    } catch (e: Exception) {
                        Log.e("Parse error", "Invalid JSON: $responseData")
                        callback(null)
                    }
                } else {
                    Log.e("API-call", "Response error with code: ${response.code}")
    ...
    fun fetchKalturaData(entryId: String, onResult: (String?) -> Unit) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                withContext(Dispatchers.Main) { onResult(responseText) }
            } catch (e: Exception) {
                Log.e("Kaltura", "Error: ${e.message}")
    ...
    fun authenticateAndLogCookies(onComplete: () -> Unit) {
        ...
        val cookieHeader =
            """ppms_privacy_6c58358e-1595-4533-8cf8-9b1c061871d0={"visitorId":"0478c604-ce60-4537-8e17-fdb53fcd5c31","domain":{"normalized":"www.kb.dk","isWildcard":false,"pattern":"www.kb.dk"},"consents":{"analytics":{"status":1}}}; CookieScriptConsent={"bannershown":1,"action":"reject","consenttime":$currentUnixTime,"categories":"[]","key":"99a8bf43-ba89-444c-9333-2971c53e72a6"}"""
            ...
            override fun onResponse(call: Call, response: Response) {
                if (response.isSuccessful) {
                    ...
                    if (authHeader != null) {
                        ...
                        Log.d("Auth", "Cookie saved: $authCookie")
                    } else {
