...
import com.example.mediatoolkit.PlayerManager.setSearchResult
...
class PlayerActivity : AppCompatActivity() {private var entryId: String? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (searchResult != null) {
            if (searchResult.kalturaId == PlaybackState.currentEntryId) {
                Log.d("PlayerActivity", "Same video, skipping reload and playback reset")
                updateUIFromMetadata()
            } else {
                Log.d("PlayerActivity", "New video, starting playback")
        ...
        if (searchResult == null && playlistItems.isNotEmpty()) {
    ...
    private fun startPlaybackIfPossible() {
        if (!parsedEntryId.isNullOrEmpty() && !parsedFlavorId.isNullOrEmpty() && !parsedFileExt.isNullOrEmpty()) {
            ...
            if (currentMediaUri == mediaUrl) {
                Log.d("PlayerActivity", "Media already loaded, skipping playback")
                return
            }
    ...
    private fun castMedia(mediaUrl: String? = null) {
        Log.d("PlayerActivity", "castMedia called with mediaUrl: $mediaUrl")
        val castSession = castContext.sessionManager.currentCastSession
        val remoteMediaClient = castSession?.takeIf { it.isConnected }?.remoteMediaClient

        if (remoteMediaClient == null) {
            Toast.makeText(this, "No active Cast session or RemoteMediaClient available", Toast.LENGTH_SHORT).show()
            return
        }

        if (!mediaUrl.isNullOrEmpty()) {
            Log.d("PlayerActivity", "Casting single media (explicit mediaUrl)")
            val item = PlaybackState.currentSearchResult
            val chromecastUrl = mediaUrl
        ...
        if (castPlaylist.isNotEmpty()) {
            ...
            suspend fun processNext(index: Int) {
                if (index >= castPlaylist.size) {
                    withContext(Dispatchers.Main) {
                        if (mediaQueueItems.isNotEmpty()) {
                            ...
                            Toast.makeText(this@PlayerActivity, "Could not create cast queue", Toast.LENGTH_SHORT).show()
                        }
                    }
                    return
                }
                ...
                ApiService.getMediaUrlviaAPI(item.kalturaId!!, forCast = true) { uri ->
                    ...
                    CoroutineScope(Dispatchers.Default).launch { processNext(index + 1) }
                }
            }

            CoroutineScope(Dispatchers.Default).launch { processNext(0) }
            return
        }

        if (parsedEntryId != null && parsedFlavorId != null && parsedFileExt != null) {
            val item = PlaybackState.currentSearchResult
            val chromecastUrl = ApiService.generateCastUrl(parsedEntryId!!, parsedFlavorId!!, parsedFileExt!!)
            Log.d("PlayerActivity", "Chromecast URL for single item (fallback): $chromecastUrl")
            ...
            Toast.makeText(this, "No media URL or metadata available for casting", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    override fun onStop() {
        super.onStop()
        Log.d("", "onStop called in PlayerActivity")
    }

    override fun onDestroy() {
        super.onDestroy()
        wakeLockManager.release()
        Log.d("", "onDestroy called in PlayerActivity")
    }
